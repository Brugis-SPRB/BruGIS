...
ux.plugins.AddLayers = Ext.extend(gxp.plugins.Tool, {
	...
	/**api: config[skipSourceIdList]
     * ``Integer``
     * The list of identifiers of the sources that we should not show by default.
	 * This is a modded feature added to the default Geoserver AddLayers.
	 * This is to mask the geowebcache which shows all the layers including the private layers that should be listed only for the authorized users
	 */
	skipSourceIdList: null,

	/**api: config[skipWorkspaceIdList]
     * ``Integer``
     * The list of identifiers of the workspace that we should not show by default.
	 * This is a modded feature added to the default Geoserver AddLayers.
	 * This is to mask the workspace that should not be visible for any reason
	 */
	skipWorkspaceIdList: null,
    ...
    constructor: function(config) {
        ...
        ux.plugins.AddLayers.superclass.constructor.apply(this, arguments);        
    },
    ...
    addActions: function() {
        ...
        var actions = ux.plugins.AddLayers.superclass.addActions.apply(this, [options]);
    ...
    initCapGrid: function() {
        ...
        var sourceToSkip = false;
		for (var id in target.layerSources) {
            source = target.layerSources[id];
            if (source.store && source.ptype !== "gxp_cataloguesource") {
				if (this.skipSourceIdList !== null) {
					for (var sourceId in this.skipSourceIdList) {
						if (this.skipSourceIdList[sourceId] == id) {
							sourceToSkip = true;
						}
					}
				}
				if (sourceToSkip == false) {
        ...
        var capGridPanel = new Ext.grid.GridPanel({
            ...
            colModel: new Ext.grid.ColumnModel([
                expander,
                {id: "title", header: this.panelTitleText, dataIndex: "title", size: 200, sortable: true},
                {id:"id", header: "Id", dataIndex: "name", size: 175, sortable: true}
            ]),
            listeners: {
        ...
        source.store.sort('name','ASC'); //Tri sur le name 
		...
		//this.capGrid.items.items[0].resizeEl = this.capGrid;
    },
